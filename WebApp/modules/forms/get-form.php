    ...
    if ($FILTER_USER == "ALL") {
        ...
        if($res['status']===null){
            ...
            foreach ($res['data'] as $ROW) {
                ...
                if ($ROW['state'] == 1){
                    $ROWDATA['STATE_TEXT'] = "Completado";
                }else{
                    $ROWDATA['STATE_TEXT'] = "Pendiente";
                }

                $ROWDATA['START_DATE'] = date("d/m/Y", strtotime($ROW['startDate']));
                $ROWDATA['END_DATE'] = date("d/m/Y", strtotime($ROW['endDate']));
                $ROWDATA['ALL_QUIZ_ASSIGNED'] = $ROW['allQuizAssigned'];
                $ROWDATA['CREATED_DATE'] = date("d/m/Y", strtotime($ROW['createdDate']));

                $ROWDATA['CATEGORY_ID'] = $ROW['category']['id'];
                $ROWDATA['CATEGORY_NAME'] = $ROW['category']['name'];
